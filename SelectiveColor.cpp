...
static float one_lut[256] = { 1.0f };
static float min_lut[256] = { 
...
static void min_med_max(const unsigned char* color, unsigned char& min, unsigned char& med, unsigned char& max)
{
...
static float adjust_component(int mode, unsigned char component_color, float omega, float adjust, float black)
{
    (void)black;
...
static void adjust_color(unsigned char* dst_color, const unsigned char* src_color, int mode, int component, float cyan, float magtenta, float yellow, float black)
{
    ...
    dst_color[SelectiveColor::Component_Red] = cv::saturate_cast<unsigned char>(src_color[SelectiveColor::Component_Red] + result[SelectiveColor::Component_Red]);
    dst_color[SelectiveColor::Component_Green] = cv::saturate_cast<unsigned char>(src_color[SelectiveColor::Component_Green] + result[SelectiveColor::Component_Green]);
    dst_color[SelectiveColor::Component_Blue] = cv::saturate_cast<unsigned char>(src_color[SelectiveColor::Component_Blue] + result[SelectiveColor::Component_Blue]);
}
...
int SelectiveColor::Adjust(cv::Mat& dst, const cv::Mat& src, int mode, int component, float cyan, float magtenta, float yellow)
{
    if (dst.type() != src.type() || dst.rows != src.rows || dst.cols != src.cols)
    {
        dst = src.clone();
    }
...
#pragma omp parallel for
    ...
    {
        int dst_step = r * static_cast<int>(dst.step);
        int src_step = r * static_cast<int>(src.step);

        for (int c = 0; c < src.cols; ++c)
        {
            adjust_color(dst_data + dst_step + c * channels, src_data + src_step + c * channels, mode, component, cyan, magtenta, yellow, 0.0f);
        }
    }
...
extern int cuda_adjust_color(cv::cuda::GpuMat& dst, const cv::cuda::GpuMat& src, int mode, int component, float cyan, float magtenta, float yellow, cv::cuda::Stream& stream);
int SelectiveColor::Adjust(cv::cuda::GpuMat& dst, const cv::cuda::GpuMat& src, int mode, int component, float cyan, float magtenta, float yellow, cv::cuda::Stream& stream)
{
    if (dst.type() != src.type() || dst.rows != src.rows || dst.cols != src.cols)
    {
        dst = src.clone();
    ...
    return cuda_adjust_color(dst, src, mode, component, cyan, magtenta, yellow, stream);
}
